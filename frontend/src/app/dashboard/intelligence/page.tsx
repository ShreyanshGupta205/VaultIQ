...
import { Zap, BrainCircuit, FileImage, ShieldAlert, Cpu, Check, Loader2, X } from "lucide-react";
    ...
    return (
        <div className="space-y-8 max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="glass-card p-6 rounded-2xl border border-primary/20 bg-primary/5">
                    ...
                    <div className="text-4xl font-bold text-foreground mb-1">
                        {insights.reduce((acc, curr) => curr.savings.includes('GB') ? acc + parseFloat(curr.savings) : acc, 0).toFixed(1)}
                        <span className="text-xl text-muted-foreground ml-1">GB</span>
                    </div>
                </div>
            ...
            <h3 className="text-xl font-semibold mb-4 border-b border-white/10 pb-4">
                {insights.length > 0 ? "Actionable Insights" : "Vault Fully Optimized"}
            </h3>
            ...
            {insights.length === 0 && !isScanning && (
                ...
                >
                    ...
                    <p className="text-muted-foreground max-w-md">VaultMind has structured your files, removed all duplicates, and secured sensitive data. No pending actions.</p>
                </motion.div>
            )}
                ...
                <AnimatePresence>
                    {insights.map((insight) => (
                        <motion.div
                            ...
                            className="glass-card p-6 rounded-2xl border border-white/5 flex flex-col md:flex-row gap-6 items-start md:items-center group hover:border-white/20 transition-all overflow-hidden"
                        >
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto shrink-0 mt-4 md:mt-0">
                                ...
                                <button
                                    onClick={() => handleFix(insight.id)}
                                    disabled={fixing === insight.id}
                                    className="w-full sm:w-auto px-6 py-2.5 rounded-xl text-sm font-medium bg-white/10 hover:bg-white/20 border border-white/10 transition-colors whitespace-nowrap disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                                    {fixing === insight.id ? (
